...
use yii\jui\timepicker\DatetimePicker;
use Exception;
...
class DatetimePickerTest extends TestCase
{
    ...
    protected function getActual($mode, $value, array $config = [])
    {
        ...
        switch ($mode) {
            ...
            case static::MODE_NAME_VALUE_AJAX:
                ...
                return DatetimePicker::widget(array_merge($config, [
                    'name' => 'datetime',
                    'value' => $value
                ]));
    ...
    protected function getIdName($mode)
    {
        switch ($mode) {
            ...
            case static::MODE_MODEL_ATTRIBUTE_VALUE:
            case static::MODE_MODEL_ATTRIBUTE_VALUE_AJAX:
                $id = 'testform-datetime';
                $name = 'TestForm[datetime]';
                return [$id, $name];
        }
        throw new Exception;
    }
...
jQuery('#$id').datetimepicker({"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss"});
EXPECTED_JS;
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss","beforeShow":function (input, inst) { return false; }});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"beforeShow":null,"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
...
EXPECTED_JS;
    ...
    public function testWidgetDateFormatFull($mode, $value)
    {
        ...
        if (is_int($value) || (is_string($value) && strlen($value)) || ($value instanceof DateTime) || ($value instanceof DateTimeInterface)) {
            $formatter = Yii::$app->getFormatter();
            $encodedValueAttr = ' value="' . Html::encode($formatter->asDate($value, 'full') . ' ' . $formatter->asTime($value)) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($formatter->asDate($value, 'yyyy-MM-dd') . ' ' . $formatter->asTime($value, 'HH:mm:ss')) . '"';
        } elseif (!is_null($value)) {
            $encodedValueAttr = ' value="' . Html::encode($value) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($value) . '"';
...
EXPECTED_JS;
    ...
    public function testWidgetTimeFormatFull($mode, $value)
    {
        ...
        if (is_int($value) || (is_string($value) && strlen($value)) || ($value instanceof DateTime) || ($value instanceof DateTimeInterface)) {
            $formatter = Yii::$app->getFormatter();
            $encodedValueAttr = ' value="' . Html::encode($formatter->asDate($value) . ' ' . $formatter->asTime($value, 'full')) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($formatter->asDate($value, 'yyyy-MM-dd') . ' ' . $formatter->asTime($value, 'HH:mm:ss')) . '"';
        } elseif (!is_null($value)) {
            $encodedValueAttr = ' value="' . Html::encode($value) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($value) . '"';
...
EXPECTED_JS;
    ...
    public function testWidgetAltDateFormatYmd($mode, $value)
    {
        ...
        if (is_int($value) || (is_string($value) && strlen($value)) || ($value instanceof DateTime) || ($value instanceof DateTimeInterface)) {
            $formatter = Yii::$app->getFormatter();
            $encodedValueAttr = ' value="' . Html::encode($formatter->asDate($value) . ' ' . $formatter->asTime($value)) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($formatter->asDate($value, 'yyyyMMdd') . ' ' . $formatter->asTime($value, 'HH:mm:ss')) . '"';
        } elseif (!is_null($value)) {
            $encodedValueAttr = ' value="' . Html::encode($value) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($value) . '"';
        } else {
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yymmdd","altSeparator":" ","altTimeFormat":"HH:mm:ss","dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
    ...
    public function testWidgetAltTimeFormatHis($mode, $value)
    {
        ...
        if (is_int($value) || (is_string($value) && strlen($value)) || ($value instanceof DateTime) || ($value instanceof DateTimeInterface)) {
            $formatter = Yii::$app->getFormatter();
            $encodedValueAttr = ' value="' . Html::encode($formatter->asDate($value) . ' ' . $formatter->asTime($value)) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($formatter->asDate($value, 'yyyy-MM-dd') . ' ' . $formatter->asTime($value, 'HHmmss')) . '"';
        } elseif (!is_null($value)) {
            $encodedValueAttr = ' value="' . Html::encode($value) . '"';
            $encodedAltValueAttr = ' value="' . Html::encode($value) . '"';
        } else {
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HHmmss","dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"numberOfMonths":3,"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"numberOfMonths":1,"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"showButtonPanel":false,"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
...
EXPECTED_HTML;
        ...
        $timeFormat = FormatConverter::convertDatePhpOrIcuToJui('medium', 'time');
        $expectedJs = <<<EXPECTED_JS
jQuery('#$id').datetimepicker({"showButtonPanel":true,"dateFormat":"$dateFormat","separator":" ","timeFormat":"$timeFormat","altField":"#$id-alt","altFieldTimeOnly":false,"altFormat":"yy-mm-dd","altSeparator":" ","altTimeFormat":"HH:mm:ss"});
EXPECTED_JS;
        $this->checkExpected($mode, $actual, $expectedHtml, $expectedJs);
    }
